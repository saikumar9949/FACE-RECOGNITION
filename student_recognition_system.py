...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'reddysaikumar931@gmail.com' ")
...
def TakeImages():
    ...
    assure_path_exists("SuspectDetails/")
    assure_path_exists("TrainingImage/")
    serial = 0
    exists = os.path.isfile("SuspectDetails\SuspectDetails.csv")
    if exists:
        with open("SuspectDetails\SuspectDetails.csv", 'r') as csvFile1:
        ...
        with open("SuspectDetails\SuspectDetails.csv", 'a+') as csvFile1:
...
def TrackImages():
    ...
    exists1 = os.path.isfile("SuspectDetails\SuspectDetails.csv")
    if exists1:
        df = pd.read_csv("SuspectDetails\SuspectDetails.csv")
    else:
        mess._show(title='Details Missing', message='Suspects details are missing, please check!')
        cam.release()
        cv2.destroyAllWindows()
        window.destroy()
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                bb = str(aa)
                bb = bb[2:-2]
                Recognised = [str(ID), '', bb, '', str(date), '', str(timeStamp)]
            else:
                Id = 'Unknown'
                bb = str(Id)
            cv2.putText(im, str(bb), (x, y + h), font, 1, (255, 255, 255), 2)
        cv2.imshow('Taking Recognised', im)#gray
    ...
    ts = time.time()
    date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
    exists = os.path.isfile("Recognised\Recognised_" + date + ".csv")
    if exists:
        with open("Recognised\Recognised_" + date + ".csv", 'a+') as csvFile1:
            writer = csv.writer(csvFile1)
            writer.writerow(Recognised)
        csvFile1.close()
    else:
        with open("Recognised\Recognised_" + date + ".csv", 'a+') as csvFile1:
            writer = csv.writer(csvFile1)
            ...
            writer.writerow(Recognised)
        csvFile1.close()
    with open("Recognised\Recognised_" + date + ".csv", 'r') as csvFile1:
        reader1 = csv.reader(csvFile1)
...
window.title("Recognised System")
window.configure(background='#F8C471')
...
message3 = tk.Label(window, text="STUDENT RECOGNITION & DATA COLLECTION APPLICATION" ,fg="white",bg="#262523" ,width=71 ,height=1,font=('times', 29, ' bold '))
...
head2 = tk.Label(frame2, text="                       Add New Student Data                       ", fg="black",bg="#3ece48" ,font=('times', 17, ' bold ') )
head2.grid(row=0,column=0)

head1 = tk.Label(frame1, text="                       Recognize verify-identify                      ", fg="black",bg="#3ece48" ,font=('times', 17, ' bold ') )
head1.place(x=0,y=0)
